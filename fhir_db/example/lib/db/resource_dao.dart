...
class ResourceDao {
  ...
  Future updatePw(String oldPw, String newPw) =>
      Db.instance.updatePassword(oldPw, newPw);

  /// accessing the actual database instance
  Future<Database> _db(String password) => Db.instance.database(password);
  ...
  /// ************************************************************************
  /// these functions are purely for testing purposes, not to be copied over
  /// for production libraries
  Future<List> resourceTypeStrings(String password) async =>
      await _getResourceTypes(password);
}
