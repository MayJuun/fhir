...
class FhirHiveDb {
  ...
  Future<void> initDb() async {
    ...
    final Box<List<Stu3ResourceType>> typesBox = await Hive.openBox('types');
    types = typesBox.get('types')?.toSet() ?? <Stu3ResourceType>{};
  }
  ...
  Future<Box<Map<String, dynamic>>> getBox(
      Stu3ResourceType resourceType) async {
    await ensureInit;
  ...
  Future<bool> addType(Stu3ResourceType resourceType) async {
    try {
      if (types.contains(resourceType)) {
        return true;
      } else {
        await ensureInit;
        final Box<List<Stu3ResourceType>> box = Hive.box('types');
        final resourceSet = box.get('types')?.toSet() ?? <Stu3ResourceType>{};
  ...
  Future<Resource?> read(Stu3ResourceType resourceType, String id) async {
    await ensureInit;
  ...
  Future<void> sinkDataToStream(Stu3ResourceType resourceType) async {
  ...
  Future<List<Resource>> readAllOneType(Stu3ResourceType resourceType) async {
    await ensureInit;
    final Box<Map<String, dynamic>> box = await getBox(resourceType);
  ...
  Future<bool> deleteType(Stu3ResourceType resourceType) async {
    try {
      final Box<Map<String, dynamic>> box = await getBox(resourceType);
      await box.clear();
      return true;
  ...
  void registerResourceTypes() {
    Hive.registerAdapter(Stu3ResourceTypeAdapter());
  }
}
