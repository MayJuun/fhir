...
class FhirHiveDb {
  ...
  Future<void> initDb() async {
    ...
    final Box<List<Dstu2ResourceType>> typesBox = await Hive.openBox('types');
    types = typesBox.get('types')?.toSet() ?? <Dstu2ResourceType>{};
  }
  ...
  Future<Box<Map<String, dynamic>>> getBox(
      Dstu2ResourceType resourceType) async {
    await ensureInit;
  ...
  Future<bool> addType(Dstu2ResourceType resourceType) async {
    try {
      if (types.contains(resourceType)) {
        return true;
      } else {
        await ensureInit;
        final Box<List<Dstu2ResourceType>> box = Hive.box('types');
        final resourceSet = box.get('types')?.toSet() ?? <Dstu2ResourceType>{};
  ...
  Future<Resource?> read(Dstu2ResourceType resourceType, String id) async {
    await ensureInit;
  ...
  Future<void> sinkDataToStream(Dstu2ResourceType resourceType) async {
  ...
  Future<List<Resource>> readAllOneType(Dstu2ResourceType resourceType) async {
    await ensureInit;
    final Box<Map<String, dynamic>> box = await getBox(resourceType);
  ...
  Future<bool> deleteType(Dstu2ResourceType resourceType) async {
    try {
      final Box<Map<String, dynamic>> box = await getBox(resourceType);
      await box.clear();
      return true;
  ...
  void registerResourceTypes() {
    Hive.registerAdapter(Dstu2ResourceTypeAdapter());
  }
}
