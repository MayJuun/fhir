...
void main() async {
  ...
  var file = File('./parsers/r5/fhir.schema.r5.json');
  ...
  for (var i = 0; i < 2; i++) {
    for (var obj in schema['definitions'].keys) {
      ...
      if (type != '' && type != 'other') {
        var fileData = (await File('./lib/r5/$type').readAsString()) + data;
        var beginning = beginnings['./lib/r5/$type'];
        if (beginning == null) print(type);
        i == 1
            ? await File('./lib/r5/$type').writeAsString(fileData)
            : await File('./lib/r5/$type').writeAsString(beginning);
      }
    }
  }
}
