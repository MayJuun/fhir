...
void main() async {
  var dir = Directory('./test/test_data/mimic-fhir-r4');
  await File('./test/test_data/test_data_errors.txt').writeAsString('');
  var string = '';
...
Future<void> writeErrorFile(String error) async {
  var errors =
      await File('./test/test_data/test_data_errors.txt').readAsString();
  errors += error;
  await File('./test/test_data/test_data_errors.txt').writeAsString(errors);
}
