...
import '../../lib/fhir_r5.dart' as fhir_r5;

void main() async {
  var dir = Directory('./test/r5/r5_examples');
  await File('./test/r5/errors.txt').writeAsString('');
  var string = '';
  for (var file in await dir.list().toList()) {
    var contents = await File(file.path).readAsString();
    var resource = fhir_r5.Resource.fromJson(json.decode(contents));
    if (resource == null) {
      print(file);
    } else {
...
Future<void> writeErrorFile(String error) async {
  var errors = await File('./test/r5/errors.txt').readAsString();
  errors += error;
  await File('./test/r5/errors.txt').writeAsString(errors);
}
